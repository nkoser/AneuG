#!/usr/bin/env python
"""Evaluate PCA-compressed vessel-aware conditional flow-matching checkpoints."""
import argparse
import csv
import json
import os
...
def parse_args():
    p = argparse.ArgumentParser("eval_vessel_pca_flow_matching")
...
def _build_model(saved_args, input_dim, device):
    return VesselAwareFlowMatching(
        ...
        blocks=int(saved_args.get("flow_blocks", 4)),
        dropout=float(saved_args.get("dropout", 0.02)),
    ).to(device)


def _encode_pca(x, pca_mean, pca_components, pca_score_scale):
    return ((x - pca_mean.to(x.device)) @ pca_components.to(x.device).t()) / pca_score_scale.to(x.device)


def _decode_pca(z, pca_mean, pca_components, pca_score_scale):
    return (z * pca_score_scale.to(z.device)) @ pca_components.to(z.device) + pca_mean.to(z.device)
...
def main():
    ...
    pca_components = ckpt["pca_components"].to(device)
    pca_score_scale = ckpt["pca_score_scale"].to(device)
    pca_dim = int(ckpt.get("pca_dim_actual", pca_components.shape[0]))
    ...
    model = _build_model(saved_args, pca_dim, device)
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            z_gt = _encode_pca(x, pca_mean, pca_components, pca_score_scale)
            x_pca_recon = _decode_pca(z_gt, pca_mean, pca_components, pca_score_scale)
            pca_recon = torch.sqrt(torch.mean((x_pca_recon - x) ** 2, dim=1) + 1e-8)
            ...
            zero_z = model.sample(
                ...
                method=args.sampler,
                x0=torch.zeros_like(z_gt),
            )
            zero_x = _decode_pca(zero_z, pca_mean, pca_components, pca_score_scale)
            zero_to_gt = torch.sqrt(torch.mean((zero_x - x) ** 2, dim=1) + 1e-8)
            zero_to_pca = torch.sqrt(torch.mean((zero_x - x_pca_recon) ** 2, dim=1) + 1e-8)
            zero_latent_to_gt = torch.sqrt(torch.mean((zero_z - z_gt) ** 2, dim=1) + 1e-8)

            cond_rep = cond.repeat_interleave(args.num_samples, dim=0)
            samples_z_flat = model.sample(
                cond_rep,
                num_steps=args.num_steps,
                temperature=args.temperature,
                method=args.sampler,
            )
            samples_z = samples_z_flat.view(x.shape[0], args.num_samples, -1).transpose(0, 1).contiguous()
            samples_x = _decode_pca(
                samples_z_flat,
                pca_mean,
                pca_components,
                pca_score_scale,
            ).view(x.shape[0], args.num_samples, -1).transpose(0, 1).contiguous()
            sample_pair_x = _pairwise_sample_rmse(samples_x)
            sample_pair_z = _pairwise_sample_rmse(samples_z)
            sample_to_gt = torch.sqrt(torch.mean((samples_x - x.unsqueeze(0)) ** 2, dim=2) + 1e-8).mean(dim=0)
            sample_to_pca = torch.sqrt(torch.mean((samples_x - x_pca_recon.unsqueeze(0)) ** 2, dim=2) + 1e-8).mean(dim=0)
            sample_to_zero = torch.sqrt(torch.mean((samples_x - zero_x.unsqueeze(0)) ** 2, dim=2) + 1e-8).mean(dim=0)
            sample_latent_to_gt = torch.sqrt(torch.mean((samples_z - z_gt.unsqueeze(0)) ** 2, dim=2) + 1e-8).mean(dim=0)

            for local_idx in range(x.shape[0]):
                rows.append({
                    "case": dataset.case_names[offset + local_idx],
                    "pca_recon_rmse": float(pca_recon[local_idx].cpu()),
                    "zero_to_gt_rmse": float(zero_to_gt[local_idx].cpu()),
                    "zero_to_pca_recon_rmse": float(zero_to_pca[local_idx].cpu()),
                    "zero_latent_to_gt_rmse": float(zero_latent_to_gt[local_idx].cpu()),
                    "sample_pair_rmse": float(sample_pair_x[local_idx].cpu()),
                    "sample_pair_latent_rmse": float(sample_pair_z[local_idx].cpu()),
                    "sample_to_gt_rmse": float(sample_to_gt[local_idx].cpu()),
                    "sample_to_pca_recon_rmse": float(sample_to_pca[local_idx].cpu()),
                    "sample_to_zero_rmse": float(sample_to_zero[local_idx].cpu()),
                    "sample_latent_to_gt_rmse": float(sample_latent_to_gt[local_idx].cpu()),
                })
            offset += x.shape[0]

    summary = {
        "ckpt": args.ckpt,
        "model_type": "pca_flow_matching",
        "condition_space": condition_space,
        ...
        "temperature": args.temperature,
        "pca_dim": pca_dim,
        "pca_total_explained": float(ckpt.get("pca_total_explained", float("nan"))),
    }
